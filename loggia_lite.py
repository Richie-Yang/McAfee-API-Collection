"""
loggia_lite.py v1.20; Author: Richie Yang; Last edited at 2021/2/18.
    ...
    def __init__(self, filename='testing'):
        ...
        try:
            ...
            print("[%s][%s][%s]Logging.__init__ error traceback: %s"
                  % (datetime.now(), "WARNING", "loggia_lite", str(trace_output)))
            print("[%s][%s][%s]Logging.__init__ execution failed: %s"
                  % (datetime.now(), "WARNING", "loggia_lite", str(file_err)))
            # traceback.print_exc()
            ...
            trace_output = traceback.print_exc()
            print("[%s][%s][%s]Logging.__init__ error traceback: %s"
                  % (datetime.now(), "ERROR", "loggia_lite", str(trace_output)))
            print("[%s][%s][%s]Logging.__init__ execution failed: %s"
                  % (datetime.now(), "ERROR", "loggia_lite", str(other_err)))
            sys.exit()
