"""
nsm_import_domain_blacklist_man.py v1.00; Author: Richie Yang; Last edited at 2021/2/20.
Compatible Platform list: McAfee NSM 9.1 and 10.1, Windows 10 20H2
Description: This script is made to remotely import domain blacklist CSV into NSM.
New Features: None
...
loggia_lite.Logging(filename='nsm_import_domain_blacklist_man')
...
usr = 'admin'
pwd = 'admin123'
whitelist_filename = "whitelist_domain.txt"
blacklist_filename = "blacklist_domain.txt"
...
'''
To read CSV file to retrieve domain blacklist, and also calculate the length of the list.
'''
...
with open(dir_to_blacklist) as file:
    ...
    for line in file_as_list:
        blacklist_domain_list.append(line)

blacklist_domain_list_len = len(blacklist_domain_list)

# pprint.pprint(blacklist_domain_list_len)
...
NSM will response with createdResourceId, which will later used in blacklist_domain_profile.
'''

blacklist_domain_processed_list = []
rule_object_id = ''

for i in range(blacklist_domain_list_len):
    rule_object_data_1 = nsm.add_rule_object(
        add_rule_object_type='HOST_DNS_NAME',
        rule_object_visible_to_child_opt=True,
        rule_object_desc='api-generated',
        rule_object_name=str(blacklist_domain_list[i]),
        rule_object_data={
            'hostDNSNameList': [
                str(blacklist_domain_list[i])
            ]},
        rule_object_type='HOST_DNS_NAME'
    )
    try:
        ...
        logging.error("nsm_import_domain_blacklist_man error traceback: %s" % str(trace_output))
        logging.error("nsm_import_domain_blacklist_man execution failed: %s" % str(err))

        rule_object_data_2 = nsm.get_rule_objects_from_domain(rule_object_type='HostDNSName')
        rule_object_data_len_2 = len(rule_object_data_2['RuleObjDef'])

        for z in range(rule_object_data_len_2):
            if rule_object_data_2['RuleObjDef'][z]['HostDNSName']['hostDNSNameList'][0] == \
                    str(blacklist_domain_list[i]) and \
                    rule_object_data_2['RuleObjDef'][z]['description'] == 'api-generated':
                rule_object_id = rule_object_data_2['RuleObjDef'][z]['ruleobjId']

    finally:
        blacklist_domain_profile = {
            'Name': str(blacklist_domain_list[i]),
            'RuleObjectId': str(rule_object_id),
            'RuleObjectType': 'HOST_DNS_NAME'
        }
        blacklist_domain_processed_list.append(blacklist_domain_profile)

logging.debug("nsm_import_domain_blacklist_man process: pending blacklist ip profiles are %s"
              % str(blacklist_domain_processed_list))
# pprint.pprint(str(blacklist_domain_processed_list))
...
logging.debug("nsm_import_domain_blacklist_man process: firewall policy summary data is %s"
...
for x in range(firewall_policy_summary_data_len):
    if firewall_policy_summary_data_obj[x]['policyName'] == policy_name:
        ...
        logging.debug("nsm_import_domain_blacklist_man process: firewall policy detail data is %s"
                      % str(firewall_policy_summary_data))
        ...
        logging.debug("nsm_import_domain_blacklist_man process: pending firewall policy rules are %s"
        ...
        for y in range(blacklist_domain_list_len):
            ...
            if matched_count == firewall_policy_member_rule_list_len:
                firewall_policy_member_rule_list['MemberRuleList'][0]['SourceAddressObjectList'].append(
                    blacklist_domain_processed_list[y]
                )
        ...
        logging.debug("nsm_import_domain_blacklist_man process: processed firewall policy rules are %s"
                      % str(firewall_policy_member_rule_list))
        # pprint.pprint(firewall_policy_member_rule_list)
